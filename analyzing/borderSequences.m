% convergence of velocity - borders not borders : qequences....
...
for type=1:4
    ...
    for sub_i=1:length(subjects)
            ...
            for i =1:size(labeled_saccade_vec,2)-1
                ...
                if length(temp)>10 && size(labeled_saccade_vec,2)>2
                    ...
                    if isempty(find(currAmp==0)) && sum(currAmp)<10
                        ...
                        if i>1
                        flag_oneBefore=labeled_saccade_vec(9,i-1);
                        else
                         flag_oneBefore=0;   
                        end
                        if flag==1 && flag_oneBefore==1
                            relD_B=relD_B+1;
                            MeanVel_B(relD_B,1:length(M))=M;%(1:10);
                            durDrift_B(relD_B)=length(M);
                            MeanVar_B(relD_B,1:length(M))=V;%(1:10);
                        else if flag==1 && flag_oneBefore==0
                            relD=relD+1;
        ...
        p2=plot(10:10:10*size(MeanVel_B,2),nanmean(MeanVel_B,1),'color',c2);
        hold on
        ...
        p1=plot(10:10:10*size(MeanVel,2),nanmean(MeanVel,1),'color',color);
        hold on
        p2=plot(10:10:10*size(MeanVel_B,2),nanmean(MeanVel_B,1),'color',c2);
        p1.Color(4) = 0.3;
        p2.Color(4) = 0.3;
    ...
    title(['N_first=' num2str(size(allSub_MeanVel_B,1)) '  N_seq=' num2str(size(allSub_MeanVel,1))])
    ...
    axis([0 10*size(allSub_MeanVel,2) 1 11])
    axis([0 100 1 11])
%     title('')
    xlabel('Time [ms]')
    ylabel('Speed [deg/sec]')
    ...
    errorbar(10:10:10*size(allSub_MeanVel,2),nanmean(allSub_MeanVel),nanstd(allSub_MeanVel)./sqrt(size(allSub_MeanVel,1)-sum(isnan(allSub_MeanVel))),'color',color,'LineWidth',2)
    hold on
    errorbar(10:10:10*size(allSub_MeanVel,2),nanmean(allSub_MeanVel_B),nanstd(allSub_MeanVel_B)./sqrt(size(allSub_MeanVel_B,1)-sum(isnan(allSub_MeanVel_B))),'color',c2,'LineWidth',2)
    title(['N_first=' num2str(size(allSub_MeanVel_B,1)) '  N_seq=' num2str(size(allSub_MeanVel_B,1))])
    plot(10:10:10*size(allSub_MeanVel,2),nanmean(allSub_MeanVel),'color',color,'LineWidth',4)
    plot(10:10:10*size(allSub_MeanVel_B,2),nanmean(allSub_MeanVel_B),'color',c2,'LineWidth',4)
    %     errorbar(10:10:100,mean(allSub_MeanVel),ste(allSub_MeanVel),'color',color)
    axis([0 10*size(allSub_MeanVel,2) 1 11])
...
axes(ha30(3));
leg=legend('Not first','First in Seq');
