...
%        labels in row 5
%        if border sacc border angle in row 6
%        if border sacc angle in row 7 
...
function [labeled_saccade_vec,sacc_time_ms,sacc_amp_degrees,sacc_vel_deg2sec,sacc_maxvel_deg2sec]=typeOfSaccade(saccade_vec,XY_vec_pix,XY_vecs_deg,imdata,res,analogType)
...
for i=1:size(saccade_vec,2)
    ...
    %     sacc_length_degrees(i)=EUDist(saccades{i}(:,1)',saccades{i}(:,end)');
...
for i=1:size(saccade_vec,2)
    ...
    saccAngle = atan2d(lastP(2)-firstP(2),lastP(1)-firstP(1));
    ...
    if minDist1<thresholdDist && minDist2<thresholdDist
        ...
        labeled_saccade_vec(6,i)=BorderAngle;
        labeled_saccade_vec(7,i)=saccAngle;
...
%     plot(closePointOnBorder_first(1),closePointOnBorder_first(2),'*r')
%     plot(closePointOnBorder_last(1),closePointOnBorder_last(2),'*b')
%     
