...
function [imdatas,chan_h_pix,chan_v_pix,chan_h, chan_v,saccade_vecs, n] =sacDiffAmos(cal,fileName,sessionNum,picName,doPlot)
...
for i=1:max(size(sessionNum))
    ...
    % FOR THE 3 SECONDS CONTROL:
%     chan_h_pix=gaze{numOfFig}(1,1:min(300,size(chan_h_pix,2)));
%     chan_v_pix=gaze{numOfFig}(2,1:min(300,size(chan_h_pix,2)));
%     chan_h_pix=gaze{numOfFig}(1,end-min(299,size(chan_h_pix,2)-1):end);
%     chan_v_pix=gaze{numOfFig}(2,end-min(299,size(chan_h_pix,2)-1):end);
